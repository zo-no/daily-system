...
## 网页联动（web/review.html）

diary-builder 的追问逻辑已在 **`web/review.html`** 中实现为网页交互，与本 Skill 共享同一套规则。
...
Step 1: web/index.html
  用户填写时间线（快捷按钮 + 手动添加）
  + 心情、今日感悟、明日计划
  → 点击"下一步"，数据存入 sessionStorage

Step 2: web/review.html  ← diary-builder 规则在此执行
  前端 JS 自动分析时间线：
    · 超过 1 小时的空白 → 【时段空白】追问
    · 含糊动词（继续/玩/弄/逛…） → 【信息模糊】追问
    · 深度工作关键词 → 【深度工作】追问
  用户选择选项或自由填写
  → 合并原始记录 + [补充] 条目
  → POST /api/submit 保存完整数据

Step 3: web/report.html
  GET /api/report/:date 获取数据
  展示：时间分类统计 + 完整时间线 + 感悟 + 明日计划
  纯展示，无需填写
```

### 追问类型与网页实现对应

| Skill 定义 | review.html 实现 |
|-----------|----------------|
| 【时段空白】间隔 > 1h | 自动检测相邻条目时差，生成选项卡片 |
| 【信息模糊】含糊动词 | 正则匹配，生成自由填写卡片 |
| 【深度工作补充】 | 正则匹配，生成自由填写卡片 |
| 睡眠时段跳过 | 22:00–09:00 且间隔 > 6h 自动跳过 |
| 用户可跳过 | 每个追问卡片有"跳过"链接 |

### 数据存储

- **路径**：`data/diary/{YYYY-MM-DD}.json`
- **格式**：原始条目 + supplement 标记的补充条目混合在 `timeline` 数组中
- **原文保护**：原始条目的 `supplement: false`，补充条目的 `supplement: true`
...
### 原文件保护（绝不修改）
- **原日记文件**（如 `2025-02-25.md`）**只读，绝不修改**
- 用户的原始记录是神圣的，一字不改

### AI 生成文件（可修改）
- **生成新文件**：`2025-02-25_AI生成.md`
- 命名规则：`{日期}_AI生成.md` 或 `{日期}_AI整理.md`
- AI 生成的文件可以被后续修改、覆盖、删除
- 文件头部应标注：`# AI 生成 - 基于 {原文件名} 整理`

### 文件结构示例
```
life-records/
├── 2025-02-25.md              # 原文件（用户手写，只读）
├── 2025-02-25_AI生成.md       # AI 整理版（可修改）
└── 2025-02-25_AI洞察.md       # day-reflection 分析（可修改）
```
...
### 读取原文件（只读）
- **路径**：`kual-obsidian/人生记录/日记/YYYY/YYYY_MM_DD.md`
- **操作**：只读取，绝不修改

### 写入 AI 生成文件（可修改）
- **路径**：`kual-obsidian/人生记录/日记/YYYY/YYYY_MM_DD_AI生成.md`
- **命名**：原文件名 + `_AI生成` 后缀
- **操作**：可以修改、覆盖、删除

### 示例流程
```
1. 读取：2026_02_25.md（用户手写，只读）
2. 分析：识别空白时段，生成追问
3. 追问：用户回答补充信息
4. 生成：2026_02_25_AI生成.md（AI 整理版，可修改）
5. 保留：原文件 2026_02_25.md 一字不改
```
