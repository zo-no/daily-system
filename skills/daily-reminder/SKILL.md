...
`daily-reminder` 仍以本地配置文件为主，但执行时遵循 tool-first 检查：

```text
1) daily_status
2) 若需要给用户返回当前可访问入口 -> daily_get_link
3) 若服务未运行且确实需要页面/接口能力 -> daily_start_service({ exposeMode: "local" })
```
...
当用户问“系统现在能不能用”或“给我入口”时：

- 状态：`daily_status`
- 链接：`daily_get_link`
- 启动：`daily_start_service`
- 停止：`daily_stop_service`
