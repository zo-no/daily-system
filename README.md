...
每晚 5 分钟自由记录，AI 自动补全不清楚的细节、整理时间线并生成复盘，早上告诉你今天该做什么。
...
22:30  AI 提醒写日记 → 打开链接自由填写（5 分钟）
                     → AI 追问补充 + 整理时间线 + 生成复盘报告
```
...
1. Skill 层（命名空间 `daily`）
2. Runtime Plugin 层（`extension/`，提供 `daily_*` tools）
...
| `/daily:builder` | 自由文本解析、追问细节、补全时间线 |
| `/daily:reflect` | 柳比歇夫统计、复盘报告 |
| `/daily:morning` | 早报、今日待办推荐 |
| `/daily:reminder` | 行为规范提醒（结合复盘结果设置提醒） |
...
├── extension/                       # Runtime Plugin（daily_* tools）
├── skills/
...
│   └── daily-reminder/SKILL.md      # /daily:reminder
├── server/server.js                 # 后端：接收打卡数据（端口 8888）
├── web/
│   ├── index.html                   # Step 1：自由输入（时间线可选）
│   ├── review.html                  # Step 2：自动解析 + 追问补全
│   └── report.html                  # Step 3：展示分析
├── workspace-template/
│   ├── config.json                  # 用户配置模板（含字段说明）
│   └── diary/人生目标.md            # 角色定义模板
└── deploy/
    ├── start.sh                     # 启动（local-first，可选 tunnel）
    ├── status.sh                    # 运行状态
    ├── stop.sh                      # 停止
    └── README.md                    # 部署详情
```
...
服务端打卡数据目录（`{install_path}/data/diary`）：

```
{install_path}/data/diary/
├── YYYY-MM-DD.json                  # 兼容旧流程的聚合文件（legacy）
└── YYYY/MM/
    ├── YYYY-MM-DD_source.txt        # 原始文本（用户输入原文）
    ├── YYYY-MM-DD_parsed.json       # 解析结果（分段、追问、回答）
    ├── YYYY-MM-DD_timeline.json     # 结构化时间线（含 supplement 标记）
    └── YYYY-MM-DD_record.json       # 汇总记录（report API 读取）
```
...
| 数据未保存 | 检查 `{install_path}/data/diary/YYYY/MM/` 下是否生成 `*_source.txt` 与 `*_record.json` |
...
- [ ] 提交打卡后 `{install_path}/data/diary/YYYY/MM/` 下生成 `source/parsed/timeline/record` 文件
